		...
		["scripts"] = {
			...
			["Rematch"] = {
				...
				[71924] = {
					["author"] = "兮兮妹-末日祷告祭坛",
					["name"] = "A拉希奥",
					["code"] = "change(next) [ self.dead ]\nif [ enemy(辛迪).active &  self(伊奇).active ]\n  ability(群殴) [ enemy.aura(黑爪).exists ]\n  ability(黑爪)\n  standby [ self.aura(昏迷).exists ]\nendif\n\n\nif [ enemy(阿莱克丝).active &  self(伊奇).active ]\n  ability(群殴) [ self.hp<338 ]\n  ability(群殴) [ enemy.aura(黑爪).exists ]\n  ability(黑爪)\nendif\n\nif [ enemy(阿莱克丝).active &  self(剥石者幼崽).active ]\nability(脚踢)\nability(迅猛突袭)\nendif\n\nif [ enemy(小死).active ]\nchange(#3) [ !self(#3).played ]\nchange(#2) \nability(偏斜)\nability(脚踢)\nability(迅猛突袭)\nendif",
				},
				[68463] = {
					["author"] = "最初的悠悠-日落沼泽",
					["name"] = "~10熊猫人烈焰之灵",
